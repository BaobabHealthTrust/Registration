...
function generateHomepage(){
    ...
    if(__$("links")){
        ...
        for(var j = 0; j < childlinks.length; j++){
            ...
            button.setAttribute("link", childlinks[j].value  /*.getAttribute("link")*/);
            if (j == 0) {
...
*/
    ...
    if(__$("navigation_links")){
        ...
        for(var j = 0; j < childlinks.length; j++){
            ...
            if (j == 0 && (childlinks[j].innerHTML.trim().toLowerCase() == "logout" ||
                    childlinks[j].innerHTML.trim().toLowerCase() == "cancel")) {
                button.className = "red left";
                button.id = "btnCancel";
                button.onclick = function(){
                    if(__$("btnCancel").getAttribute("link") != null){
            ...
            } else if ((j == 1 && childlinks[j].innerHTML.trim().toLowerCase() == "finish") || j == 0 ||
                childlinks[j].innerHTML.trim().toLowerCase() == "finish") {
                button.className = "green";
                button.id = "btnNext";
